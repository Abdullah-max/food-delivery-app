...
export class Profile extends Component {
  ...
  //         revenue:
  //           (Number(data?.klayuniverse.usd) / initialTokenValuelUsd) * 100,
  ...
  handleGetResult = async () => {
    let res = await fetch(
      "https://openapi.dcoin.com/open/api/market"
    )
      .then((response) => response.json())
      .then((data) => {
        const { user } = this.props.authState;
        ...
        this.setState({
          result: Number(data?.data.helmusdt) * balance,
          resultKrw: Number(data?.data.helmusdt) * balance * 1422.81,
          revenue:
            (Number(data?.data.helmusdt) / initialTokenValuelUsd) * 100,
          kutBalance: 1 * balance,
          walletAddress: address,
          username: username,
          private_key: privatekey,
      ...
      <div className="container">
        <div className="main">
          ...
          <div className="profile">
            ...
            <div className="balance">{`${this.state?.resultKrw} KRW  `}</div>
